...
# --- MF ---
	...
	# alpha div
		...
		MF_alpha_model <- ggplot(MF_grassland2, aes(x = year_fac, y = n_taxa)) +
							...
							geom_smooth( aes(x = year_tofit, y = .fitted),se = F, span = 3, size = 0.6, col = "grey50") +		
							geom_boxplot(aes(x = year_fac, y = n_taxa, fill = treat)) +		
							ylab ("Soil Macrofauna\nAlpha diversity") +
							xlab("Year") +
							ylim(0,15) +
							scale_fill_manual(name = "Treatment", values = c("R" = "#EC9755", "RC"="#833F0B"), labels = c("Fine Earth","Fine Earth + Compost"))  +
							scale_colour_manual(name = "Treatment", values = c("R" = "#EC9755", "RC"="#833F0B"), labels = c("Fine Earth","Fine Earth + Compost"))  +
							theme_classic() +
	...
	# beta div
		...
		MF_beta_model <- ggplot(beta_div_plot_MF_long_ok2, aes(x = year1_fac, y = dist, fill = treat1)) +
							...
							geom_line( aes(x = year1_tofit, y = .fitted), se = F, span = 3, size = 0.6, col = "grey50") +		
							geom_boxplot(aes(x = year1_fac, y = dist)) +		
							ylab ("Beta diversity") +
							xlab("Year") +
							ylim(0, .70) +
	...
	# gamma div
		...
		MF_gamma_model <- ggplot(MF_grassland_agg_trt2, aes(x = year, y = n_taxa)) +
							...
							geom_smooth( aes(x= year, y = .fitted) ,se = F, span = 3, size = 0.6, linetype = "dashed", col = "grey50") +	
							#geom_line( aes(x= year, y = .fitted) , size = 0.6, linetype = "dashed") +							
	...
	# abundance
		...
		MF_ab_model <- ggplot(MF_grassland3, aes(x = year, y = ab)) +
						...
						geom_smooth( aes(x= year, y = .fitted), se = F, span = 3, size = 0.6, col = "grey50") +
						geom_point(aes(x = year, y = ab, shape = treat, fill = treat, col = treat) , size = 2.5) +								
...
# --- plants ---
	...
	# alpha div
		veg_alpha_model <- ggplot(veg_grassland2, aes(x = year_fac, y = n_taxa)) +
							geom_boxplot(aes(x = year_fac, y = n_taxa, fill = treat)) +
							geom_smooth( aes(x = year_tofit, y = .fitted) ,se = F, method = "lm", size = 0.6, linetype = "dashed" , col = "grey50") +	 #505743	
							geom_boxplot(aes(x = year_fac, y = n_taxa, fill = treat)) +		
							ylab ("Plant\nAlpha diversity") +
							xlab("Year") +
							ylim(0,30) +
	...
	# beta div
		veg_beta_model <- ggplot(beta_div_plot_veg_long_ok2, aes(x = year1_fac, y = dist)) +
							...
							geom_smooth( aes(x= as.numeric(year1_fac), col = treat1), se = F, method = "lm", size = 0.6, linetype="dashed", col = "grey50") +		
							geom_boxplot(aes(x = year1_fac, y = dist, fill = treat1)) +		
							ylab ("Beta diversity") +
							xlab("Year") +
							ylim(0, .70) +
	...
	# abundance
		veg_ab_model <- ggplot(veg_bm_2, aes(x = year, y = drymass, fill = treat, col = treat)) +
							geom_point(aes(x = year, y = drymass, shape = treat) , size = 2.5) +
							geom_smooth( aes(x = year,y = .fitted ) , se = F, span = 3, size = 0.6) +		#, linetype="dashed"
